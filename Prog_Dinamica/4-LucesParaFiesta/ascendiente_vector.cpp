...
Estado resolver(const vector<int>& potencias, const vector<int>& costes, int N, int maxPower, int minPower) {
   ...
   vector<Estado> v(maxPower+1, peor);

   v[0] = {0, 0};
   for (int i = 0; i < N; ++i)
   {
      for (int j = potencias[i]; j <= maxPower; ++j)
      {
         if(v[j - potencias[i]].coste != Infinito){
            Estado candidato;
            candidato.coste = v[j-potencias[i]].coste + EntInf(costes[i]);
            candidato.potencia = v[j-potencias[i]].potencia + potencias[i];

            if(mejor(candidato, v[j])){
               v[j] = candidato;
            }
