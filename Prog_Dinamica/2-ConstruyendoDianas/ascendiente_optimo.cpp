...
Solucion resolver(const vector<int>& D, const int& O, const int& n, vector<EntInf>& v) {
   v[0] = 0; 
   for (int i = 1; i <= n; i++)
   {
      int diana = D[i-1];
      for (int j = diana; j <= O; ++j)
      {
         v[j] = min(v[j], v[j-diana] + 1);
      }
   }
   return {v[O]};
}
...
camino reconstruir(const vector<EntInf>& v, const vector<int>& D, int n, int O){
   camino sol;

   if(v[O] != Infinito){
      int i = n, j = O;
      while(j > 0){
         if(D[i-1] <= j && v[j] == v[j-D[i-1]] + 1){
            sol.push_back(D[i-1]);
            j = j - D[i-1];
         }
         else{
...
bool resuelveCaso() {
   ...
   vector<EntInf> v(O+1, Infinito);

   Solucion sol = resolver(D, O, n, v);
   if(sol.sol != Infinito){
      sol.sol.print();
      cout << ": ";
      camino reconstruccion = reconstruir(v, D, n, O);
